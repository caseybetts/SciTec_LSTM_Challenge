# data_docker.py - Docker-optimized data processing
import pandas as pd
...
def load_and_preprocess_data(config):
    """Load and preprocess data with Docker-friendly paths"""
    ...
    model_dir = os.path.dirname(config.get("sensor_encoder_path", "sensor_encoder.pkl"))
    os.makedirs(model_dir, exist_ok=True)

    # Save encoder and scaler for inference
    with open(config.get("sensor_encoder_path", "sensor_encoder.pkl"), "wb") as f:
        pickle.dump(sensor_encoder, f)
    with open(config.get("scaler_path", "scaler.pkl"), "wb") as f:
...
def make_windows(group_df, seq_len):
    ...
    if "reentry_phase" in group_df.columns:
        ...
        labels = np.zeros(len(group_df), dtype=int)  # Default for inference
...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert y.shape == (self.seq_len,), f"Each sequence should have per-step labels, got {y.shape}"
